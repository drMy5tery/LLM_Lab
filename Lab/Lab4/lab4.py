...
MODELS = {
    "FLAN-T5": {
        ...
        "type": "seq2seq",
        "language": "English",
        "description": "FLAN-T5 Large is a fine-tuned T5 model optimized for instruction-following tasks. Trained on a diverse English corpus, it excels in QA, translation, and summarization."
    },
    "MuRIL": {
    ...
    "FrenchSum": {
        ...
        "type": "seq2seq",
        "language": "French",
        "description": "A T5 model fine-tuned for abstractive text summarization in French."
    }
}
...
def is_question_relevant(question, context, lang):
    ...
    question_lower = question.lower() if lang != "Kannada" else question
    context_lower = context.lower() if lang != "Kannada" else context
    keywords = {
        "English": ["culture", "karnataka", "bengaluru", "literature", "dance", "yaks", "summari"],
        "Kannada": ["ಸಂಸ್ಕೃತಿ", "ಕರ್ನಾಟಕ", "ಬೆಂಗಳೂರು", "ಸಾಹಿತ್ಯ", "ನೃತ್ಯ", "ಯಕ್ಷಗಾನ", "ಸಂಕ್ಷಿಪ್ತ"],
        "French": ["culture", "française", "paris", "littérature", "art", "cuisine", "impressionnisme", "résumer"]
    }
...
def clean_answer(answer):
    ...
    return re.sub(r'<extra_id_\d+>|<pad>|<s>|</s>', '', answer).strip()
...
def answer_question(model_name, context, question, lang):
    ...
    if model_info["type"] == "seq2seq":
        ...
        is_summarization = "summari" in question.lower() or "résumer" in question.lower() or "ಸಂಕ್ಷಿಪ್ತ" in question.lower()
        if model_name == "FLAN-T5":
            if is_summarization:
                prompt = f"Provide a concise summary of the following document in 3-5 sentences:\n{context[:4000]}"
            else:
                prompt = f"Given the context: {context[:4000]}\nAnswer the question: {question}\nAnswer:"
        elif model_name == "FrenchSum":
            if is_summarization:
                prompt = f"summarize: {context[:4000]}"
            else:
                prompt = f"question: {question} context: {context[:4000]} Répondez en français:"
...
def main():
    ...
    - **MuRIL** (Kannada, Indic)
    - **FrenchSum** (French, International)
    ...
    if "questions" in st.session_state and all(pdf_files.values()):
        ...
        references = {
            "English": "The document discusses text summarization using the T5 model, covering its architecture, training objectives, applications, evaluation metrics, case studies, challenges, and future directions.",
            "Kannada": "ಕರ್ನಾಟಕದ ಸಂಸ್ಕೃತಿಯು ಸಾಹಿತ್ಯ, ಸಂಗೀತ, ನೃತ್ಯ ಮತ್ತು ಆಹಾರವನ್ನು ಒಳಗೊಂಡಿದೆ, ಬೆಂಗಳೂರು ರಾಜಧಾನಿಯಾಗಿದೆ.",
            "French": "Le document décrit la culture française, incluant la littérature, l'art, la cuisine, et des éléments comme l'impressionnisme, avec Paris comme capitale."
        }
            ...
            with st.spinner(f"Processing {lang} with {model_name}..."):
                try:
                    ...
                    comparison_table[model_name] = [
                        "✓✓✓" if fluency >= 4 else "✓✓",
                        "✓✓✓" if relevance >= 4 else "✓✓",
                        "High" if metrics["rouge"]["rougeL"] > 0.5 else "Medium",
                        "✓✓✓" if question else "✓✓",
